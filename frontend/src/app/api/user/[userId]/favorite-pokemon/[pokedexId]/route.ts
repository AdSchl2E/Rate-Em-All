...
export async function POST(request: Request, context: { params: { userId: string, pokedexId: string } }) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ message: "Failed to favorise Pokémon", error: errorText }, { status: 400 });
    }

    return NextResponse.json({ message: "Favorite saved successfully" }, { status: 200 });
  } catch (error) {
